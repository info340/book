...
### Handling Events {-}
jQuery also provides _convenience_ methods for registering event listeners:
...
By using a method such as `.click(callback)` rather than `.addEventListener('click', callback)`, you avoid potential hard-to-catch bugs that may get introduced from misspelling `'click'`&mdash;rather than having the browser listen for `'clik'` events and then never seeming to respond to your actions, it will instead report an error (`.clik()` is not a known function!)
