...
- Note that in order for the `<Redirect>` to work, you need to _render_ it (e.g., return it from a Component's `render()` function). Thus a good way to programmatically redirect is to specify a `state` variable (e.g., `shouldRedirect`), and then conditionally render the `<Redirect>` if that state variable becomes true:
