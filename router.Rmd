...
## React-Router
Third-party libraries such as [**React Router**](https://reactrouter.com/docs/en/v6) provide Components that include this "matching" functionality, allowing you to easily develop single-page applications.

<p class="alert alert-warning">This chapter details how to use [**version 6**](https://reactrouter.com/docs/en/v6) of React Router, released in _November 2021_. This version is significantly different from the previous versions (including v5). Be careful when looking up examples and resources that they're utilizing the same version as you! For details about earlier versions of the React Router, see [the documentation for those versions](https://v5.reactrouter.com/)</p>

As with other libraries, you [begin using React Router](https://reactrouter.com/docs/en/v6/getting-started/tutorial) by installing the `react-router-dom` library (the browser-specific version of React Router):
...
You specify _where_ in the parent element the child element will render using an [**`<Outlet>`**](https://reactrouter.com/docs/en/v6/api#outlet) Component. This component will be replaced by the element of whichever child route matches the URL segment:
...
If you wish to show a "default" child Component when there is no further segment, you can give the child Route the [**`index`**](https://reactrouter.com/docs/en/v6/getting-started/tutorial#index-routes) prop:
...
You can access the values assigned to the URL parameters by using the [**`useParams`**](https://reactrouter.com/docs/en/v6/api#useparams) hook provided by `react-router`. This hook returns an object whose keys are the parameter names and whose values are the param values:
...
If you want to work with **query parameters** (e.g., the `?key1=value1&key2=value2` part of the URL), you can use the [`useSearchParams()`](https://reactrouter.com/docs/en/v6/api#usesearchparams) hook. This works similar to the `useState()` hook, except that the value will be stored in the URL query parameter rather than in the Component's state. Note that query parameters should only be used for values such as search queries that _don't correspond to a consistent resource_.
...
Instead, React Router provides a [**`<Link>`**](https://reactrouter.com/docs/en/v6/api#link) element that you can use to create a hyperlink to another route within the application. This component takes a **`to`** prop that you use to specify the route that it links to:
...
The primary tool for this is the [**`useNavigate()`**](https://reactrouter.com/docs/en/v6/api#usenavigate) hook. This hook, when called, will provide a function (conventionally named `navigate`) that can be called to redirect the App&mdash;to change the URL without reloading the page (and thus cause the Routes to re-render):
...
Alternatively, you can cause the route to change by rendering a [**`Navigate`**](https://reactrouter.com/docs/en/v6/api#navigate) element. This element accepts a `to` prop just like a `<Link>`, and when rendered will redirect the user as if the link were clicked without the user doing anything.
...
1. You can use a [`<HashRouter>`](https://reactrouter.com/docs/en/v6/api#hashrouter) instead of a `<BrowserRouter>` The `<HashRouter>` uses the [fragment identifier](https://en.wikipedia.org/wiki/Fragment_identifier) portion of the URI to record and track which route the user is viewing: the HTTP request is thus sent to `https://domain.com/index.html#/about` to get the `/about` route&mdash;and since `index.html` is the default resource, this can be abbreviated to `https://domain.com/#/about`, which is _almost_ as good. In this way you are always requesting the appropriate resource (`/index.html`), but can still perform client-side routing. The trade-off is that your URLs will have extraneous `#` symbols in them (which also makes utilizing inner-page navigation with the fragment more difficult), and going to `domain.com/about` will _still_ cause a 404 error. It's recommend that you **avoid** this element if possible.
