...
React's `React.createElement()` lets you create an element (a "React element", not a pure DOM element&mdash;they are different!) by specifying the element type/tag, the content, and an object that maps from attribute names to values&mdash;all as a single statement. The **`ReactDOM.createRoot()`** function creates a context in which React element can then be rendered (shown on the screen)&mdash;usually a `div#root` on the page. The root's `render()` method will then take a particular React element and insert it as the child content of the React root element . Note that `root.render()` function _replaces_ the content of the root DOM element&mdash;it's a "replace" not an "append" operation.
...
For example, a common pattern is to "wrap" a rendered `<App>` component in a `<React.StrictMode>` component:
...
The [**`StrictMode`**](https://reactjs.org/docs/strict-mode.html) component is provided by React (it's a property of the `React` object imported in the first line of the above example). Like the `use strict` declaration for JavaScript, this component enables a number of additional error checking that React can perform. While most of the issues it identifies are due to using out-of-date practices so you may not see any of its warnings, but it's still a good idea to include it when developing!

See [_Props and Composition_](#props-composition) below for more even examples of common ways that React components are composed.
...
In general function components are sufficient for defining most React components; you only need to use classes when you want to specify and use any additional methods (and there are ways to do that when using function components as well). See [React Class Components](#class-components) for more details about working with class components (particularly for handling their _state_).
