...
2. Component functions take in a single argument representing the [props](#props) (discussed below), which is conventionally named as such. While it's possible to omit this argument if it isn't used (as you can with all JavaScript functions), best practice is to always include it.
...
function MessageList(props) {
    const msgItems = props.messages.map((msgObject) => {
...
//Define and pass a prop to the parent comment when rendering it
const root = ReactDOM.createRoot(document.getElementById('root'));
root.render(<MessageList messages={messageObjArray});
```
