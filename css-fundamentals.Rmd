...
<p class="alert alert-warning">Note that not all properties or values be effectively or correctly supported by all browers. Be sure and check the browser compatibility listings!</p>
...
.alert { /* applies to all with class="alert" */
  font-size: larger;
}

.success { /* applies to all with class="success" */
...
<p class="alert alert-warning">Precedence rules are **not** a reason to prefer `#id` selectors over `.class` selectors! Instead, you can utilize the more complex selectors described in the next chapter to be able to create reusable rules an avoid duplicating property declarations.</p>
