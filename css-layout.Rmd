...
<!-- parent/container and item/child -->

<!-- container has `display:flex`
item has additional properties to control its positioning.

Container to specify layout
`flex-direction`
`justify-content`
`align-items`

Child to define sizing
`flex-grow` (extra space used)
`flex-shrink` (same, but other end--0 to not shrink)
`flex-basis` for "default size" (e.g., 25%)
-- can assign all these as once (grow, shrink, basis)
...
(code pen example (basic three-column)) -->
