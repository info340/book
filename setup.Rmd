...
<p class="alert alert-warning">If you have an older version of Node and `npm` (before Node 6.10 or `npm` 5.1), you will need to update them. You can re-install the latest version of Node, or just upgrade `npm` using the command **`npm install npm@latest -g`**. See below for an explanation of this command.</p>
...
Be sure to `link` any global packages before you run `npm install` so you don't any packages from `package.jon` that you already have!
