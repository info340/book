...
- A Bash terminal (e.g., [**Git Bash**](https://git-scm.com/downloads) for Windows, or the [**subsystem for Linux**](https://msdn.microsoft.com/en-us/commandline/wsl/install_guide); Terminal is already installed on Macs)
...
On Windows, we recommend using [**Git Bash**](https://git-scm.com/downloads), which you should install along with `git` (see below). Open this program to open the command-shell. _Alternatively_, the 64-bit Windows 10 Anniversary update (August 2016) does include a version of an integrated Bash shell. You can access this by enabling the [**subsystem for Linux**](https://msdn.microsoft.com/en-us/commandline/wsl/install_guide) and then running `bash` in the command prompt.
...
<p class="alert alert-warning">If you have an older version of Node and `npm` (before Node 8.11 or `npm` 5.6), you will need to update them. You can re-install the latest version of Node, or just upgrade `npm` using the command **`npm install npm@latest -g`**. See below for an explanation of this command.</p>
