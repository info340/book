...
- On a Mac you'll want to use the built-in app called **Terminal**. You can open it by searching via Spotlight (hit Cmd (`⌘`) and Spacebar together, type in "terminal", then select the app to open it), or by finding it in the Applications/Utilities folder.
    ...
    As of macOS Catalina, Macs use **zsh** as the default command shell with Terminal. This works mostly the same as Bash (supporting generally the same commands). It is also possible to [switch between the two shells](https://www.howtogeek.com/444596/how-to-change-the-default-shell-to-bash-in-macos-catalina/) if needed.
...
Installing Node also installs an additional command line program called [**`npm`**](https://www.npmjs.com/). `npm` is a **package manager**, or a program used to "manage" other programs&mdash;think of it as a command line "app store" for developer tools and libraries. `npm` is the most common way of installing and running a large number of tools used in professional web development. As of September 2026, the `npm` "registry" included more than 4 million different packages.

You will need to have at least `v11` of `npm` installed. Installing the latest Node should give you the latest `npm` as well, but you can also upgrade `npm` using the command **`npm install npm@latest -g`**. See below for an explanation of this command.
...
```bash
# syntax to locally install package with npm
npm install PACKAGE-NAME
```
For example, you could install the `lodash` library using:

```bash
npm install lodash
```

This command will download the [`lodash`](https://lodash.com/) code library (a set of useful JavaScript functions). This package will be placed into a new folder _in the current project directory_ called **`node_modules/`**, and can be imported and used in the current directory's code. This is called a **local install** because the package is only available to the "local" project). You will need to install local packages once per project.

<p class="alert alert-info">Unlike with R or Python packages, Node packages are most commonly installed locally "per project" locally. This means that every projecton your computer might have its own separate copy of `lodash`. While this may seem redundant, it allows projects to be more isolated and transferable, and to enable different projects to use different versions of libraries easily.</p>
...
(Most project you work with will already have a `package.json` file, but if needed you can create one of these files by using the command `npm init` in the current project directory, and then following the instructions to fill in the fields).
...
You can run the `npm install` command as many times as you wish: if the package has already been installed (into the `node_modules/` folder), it won't be downloaded again. Only "missing" packages&mdash;such as those recently added to the `package.json` file&mdash;will be installed. Don't worry about running the install multiple times!
...
While the `package.json` file lists the desired dependencies for a project, the **`package-lock.json`** file lists the specific versions of dependencies (and the dependencies of the dependencies!) that have actually been installed. This is to make sure that if a package receives a minor update (e.g., from 4.17.18 to 4.17.19) that has a bug, you won't accidentally install that version and break your project. The `package-lock.json` file is automatically generated every time you install or re-install a local package. Changes to this file are a good way to check if you've accidentally installed a different version of a dependency.
...
### Installing global software with npm {-}

You can also use the `npm` program to download and install command line programs that can be accessed from anywhere on your computer. You install these by name using the `-g` argument to indicate that they are **globally** installed:
...
For example, you could install the [`live-server`](https://www.npmjs.com/package/live-server) package to run a local development server:

```bash
# globally install the live-server package
npm install -g live-server
```

Importantly, note the included **`-g`** option. This tells `npm` that the package should be installed globally, making it available across the entire computer, rather than just from a particular folder. Because you want to be able to use a command line program like `live-server` from any folder (e.g., for any project), command line utilities are always installed globally with the `-g` option.

<p class="alert alert-success">In general: _libraries_ (code packages that will be used by your program) are installed locally; _applications_ (programs that are executed on their own) are installed globally.</p>
...
If your computer requires administrator permissions to install a global package, you will need to use the **`sudo`** (**s**uper**u**ser **do**) command to run the install as an administrator. You do this by putting `sudo` in front of the command you want to run, for example:

```bash
# as an administrator, globally install the live-server package
...
Note that you should only use sudo when absolutely necessary (e.g., when you hit this error); don't run every command as an administrator! You should _never_ install local dependencies as an administrator; only use `sudo` when installing applications globally (with `-g`).

After an application is installed through `npm`, you can run that program from the command line by typing its name followed by any arguments. For example, you can have the live-server program print its version:

```bash
# get the version of the installed live-server application
live-server --version
```

`npm` also comes with an additional program called [**`npx`**](https://medium.com/@maybekatz/introducing-npx-an-npm-package-runner-55f7d4bd282b). This application lets you "download and run" an application without needing to install it separately. For example, you could download and run the `Vitest` testing program to see it's version:

```bash
# download and run the Vitest application without installing it separately
npx vitest --version
```

To sum up, you will use three commands with `npm` to install packages:

1. `npm install` to _locally_ install all of the dependencies for a project you check out.
2. `npm install PACKAGE-NAME` to _locally_ install a new code package and record it in the `package.json` file.
3. `npm install -g PACKAGE-NAME` to _globally_ install command line applications.

<p class="alert alert-info">While `npm` is the most popular package manager (and the one used in this course), there are others as well. For example, [**Yarn**](https://yarnpkg.com/en/) is a package manager created by Facebook that is [compatible](https://yarnpkg.com/en/docs/migrating-from-npm) with `npm`. Note that you will generally need to use one package manager or other; don't try to mix them in a single project!</p>
...
After you install `git`, you will need to do a few small configuration commands. You only need to execute these commands once per machine:
...
# Set git to always merge commits and not rebase
# This will prevent history from being changed and your work being lost
