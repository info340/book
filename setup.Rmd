...
- [**_npm_**](https://www.npmjs.com/) as a JavaScript package manager
- [**Git Bash**](https://git-scm.com/downloads) as a bash terminal (for windows users, already installed on macs)
...
In order to get the linter set up, you'll also need to globall install `jshint` with this command on the terminal (more info on using `npm` below):
...
// Remove this comment and the comments at the end of each line -- these are for informational purposes, but will not work in your settings
{
  ...
  "jshint.enable": true,              // enable the eshint linter
    "jshint.options": {
...
While we suggest using VSCode for this course, one of these other editors may be of interest to you.
