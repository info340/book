...
- Every HTML element can include an **`id`** attribute, which is used to give them a unique identifier so that we can refer to them later (e.g., from CSS or JavaScript). `id` attributes are named like variable names, and must be **unique** on the page.
    ...
    Note that, when specifying an id (i.e., `<h1 id="title">`) you do not include the `#` symbol. However, to _link_ to an element with id `title`, you include the `#` symbole before the id (i.e., `<a href="#title">)
...
Note, while different element types have different default displays (`block` v.s. `inline`), we can change how an element is displayed using CSS (more on this later).
...
### The `<body>` Section {-}
The `<body>` section of your page contains all elements that you want to render for your user. The content in your `<body>` section should be organized into a variety of _**semantically meaning**_ sections (more on this in [this chapter](standards-and-accessibility.html)). These descriptive containers aren't explicitly visible to your user, but allow you to organize your content into meaningful sections, allowing screen readers to process the information, and making your code _much_ more interpretable:
...
<html lang="en">
    <head>
        <!-- Declare meta information about your page as described above-->
    </head>
    <body>
        <!-- Page header -->
        <header>            
...
You'll populate the sections above (i.e., `<header>`, `<main>`, `<footer>`) with your desired content using desired HTML tags. 
...
You can use this to start off every web page you ever create from now on! Alternatively, you can use the Visual Studio Code shortcut of writing an exclamation point (`!`) then hitting the `tab` key in a `.html` file to create this skeleton.
